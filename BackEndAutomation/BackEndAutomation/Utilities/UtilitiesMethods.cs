...
namespace BackEndAutomation.Utilities
{
    public class UtilitiesMethods
    {
        ...
        public static void AssertNotEmpty<T>(T actual, string message, ScenarioContext scenarioContext)
        {
            ExtentTest _test = scenarioContext.Get<ExtentTest>("ExtentTest");

            if (message.Length < 0)
            {
                Logger.Log.Error(message + $"\nExpected not to be empty, but was: {actual}");
                _test.Log(Status.Fail, message + $"\nExpected not to be empty, but was: {actual}");
                Assert.Fail(message + $"\nExpected not to be empty, but was: {actual}");
            }
        ...
        /// <summary>
        /// Log message into report and test run log file
        /// </summary>
        /// <param name="message">Message to be logged</param>
        /// <param name="scenarioContext">Scenario Context</param>
        /// <param name="status">Type of log message, e.g. Info, Warning, Debug</param>
